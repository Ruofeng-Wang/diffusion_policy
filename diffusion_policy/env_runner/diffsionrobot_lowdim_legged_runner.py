...
class LeggedRunner(BaseLowdimRunner):
    ...
    def run(self, policy: BaseLowdimPolicy, online=False, generate_data=False):
        ...
        state_history = torch.zeros((env.num_envs, history*2+1, env.num_obs), dtype=torch.float32, device=device)
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                if online:    
                    obs_dict = {"obs": state_history[:, -6:, :]}
        
                    try:
                        action_dict = policy.predict_action_init(obs_dict, action_dict["action_pred"][:,6:,:])
                    except Exception as e:
                        obs_dict = {"obs": state_history[:, -12:-6, :]}
